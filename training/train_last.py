...
identical config, and log a comparison row per model. 
Features Live W&B Cloud Syncing: Overwrites last.pt/best.pt silently every epoch to save storage while preventing crash data loss.
"""
import os
import csv
import json
...
def main():
    ...
    ap.add_argument("--close_mosaic", type=int, default=10) # <-- Exposed so you can set to 0
    ...
    for sz in args.sizes:
        ...
        try:
            ...
            # --- THE LIVE CLOUD BACKUP FIX ---
            # Tell W&B to watch the weights folder. Every time a .pt file is updated,
            # W&B streams the update and overwrites the cloud version. 
            # If the cloud computer explodes at Epoch 87, best.pt and last.pt will be safe in W&B.
            wandb.save(os.path.join(weights_dir, "*.pt"), base_path=run_dir, policy="live")
            ...
            if os.path.exists(last):  
                ...
                model.train(
                    ...
                    multi_scale=False,
                    scale=(0.25, 1),
                    close_mosaic=args.close_mosaic, # <-- Controlled dynamically via args now
            ...
            best = best if os.path.exists(best) else os.path.join(weights_dir, "last.pt")
